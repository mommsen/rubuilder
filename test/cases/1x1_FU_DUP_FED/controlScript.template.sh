...
if test $nbEvtsBuilt -ne 65
then
  echo "Test failed"
  exit 1
fi

faultDetected=`getParam FU0_SOAP_HOST_NAME FU0_SOAP_PORT rubuilder::fu::Application 0 faultDetected xsd:boolean`
echo "FU0 faultDetected=$faultDetected"
if test $faultDetected != "true"
then
  echo "Test failed"
  exit 1
fi
