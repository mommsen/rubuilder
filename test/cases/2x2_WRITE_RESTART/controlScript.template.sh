...
setParam BU1_SOAP_HOST_NAME BU1_SOAP_PORT rubuilder::bu::Application 1 rawDataDir string $testDir
setParam BU0_SOAP_HOST_NAME BU0_SOAP_PORT rubuilder::bu::Application 0 metaDataDir string $testDir
setParam BU1_SOAP_HOST_NAME BU1_SOAP_PORT rubuilder::bu::Application 1 metaDataDir string $testDir
...
sendSimpleCmdToApp BU1_SOAP_HOST_NAME  BU1_SOAP_PORT pt::atcp::PeerTransportATCP 6 Configure
...
sendSimpleCmdToApp BU1_SOAP_HOST_NAME  BU1_SOAP_PORT pt::atcp::PeerTransportATCP 6 Enable
...
for i in 1 2 ; do
    ...
    setParam BU1_SOAP_HOST_NAME BU1_SOAP_PORT rubuilder::bu::Application 1 runNumber unsignedInt $runNumber
    ...
    if test $nbFilesWritten -lt 8
    then
        echo "Test failed"
        exit 1
    fi
    
    nbFilesWritten=`getParam BU1_SOAP_HOST_NAME BU1_SOAP_PORT rubuilder::bu::Application 1 nbFilesWritten xsd:unsignedInt`
    echo "BU1 nbFilesWritten=$nbFilesWritten"
    if test $nbFilesWritten -lt 8
    then
        echo "Test failed"
        exit 1
    ...
    sendSimpleCmdToApp BU1_SOAP_HOST_NAME BU1_SOAP_PORT rubuilder::bu::Application  1 Stop
    ...
    ls -l $testDir/BU-001/run$runNumber
    
    if test -x $testDir/BU-001/run$runNumber/open
    then
        echo "Test failed: $testDir/BU-001/run$runNumber/open exists"
        exit 1
    fi
    
    if test ! -s $testDir/BU-001/run$runNumber/EoLS_0001.jsn
    then
        echo "Test failed: $testDir/BU-001/run$runNumber/EoLS_0001.jsn does not exist"
        exit 1
    fi
    
    if test ! -s $testDir/BU-001/run$runNumber/EoR_$runNumber.jsn
    then
        echo "Test failed: $testDir/BU-001/run$runNumber/EoR_$runNumber.jsn does not exist"
        exit 1
    fi
