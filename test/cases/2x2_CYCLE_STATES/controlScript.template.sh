...
{
    response=`sendSimpleCmdToApp EVM0_SOAP_HOST_NAME EVM0_SOAP_PORT rubuilder::evm::Application 0 $1`
...
{
    response=`sendSimpleCmdToApp RU0_SOAP_HOST_NAME RU0_SOAP_PORT rubuilder::ru::Application 0 $1`
    if [[ ! $response =~ stateName=\"$2\" ]]
    then
    ...
    response=`sendSimpleCmdToApp RU1_SOAP_HOST_NAME RU1_SOAP_PORT rubuilder::ru::Application 1 $1`
    if [[ ! $response =~ stateName=\"$2\" ]]
    then
...
{
    response=`sendSimpleCmdToApp BU0_SOAP_HOST_NAME BU0_SOAP_PORT rubuilder::bu::Application 0 $1`
    if [[ ! $response =~ stateName=\"$2\" ]]
    then
    ...
    response=`sendSimpleCmdToApp BU1_SOAP_HOST_NAME BU1_SOAP_PORT rubuilder::bu::Application 1 $1`
    if [[ ! $response =~ stateName=\"$2\" ]]
    then
...
echo "EVM0 ruBuilderIsFlushed=$ruBuilderIsFlushed"
...
sendCmdToRUs Stop Clearing
sendCmdToBUs Stop Clearing
sleep 1
...
sendCmdToEVM Blocked FlushFailed
state=`getParam EVM0_SOAP_HOST_NAME EVM0_SOAP_PORT rubuilder::evm::Application 0 stateName xsd:string`
echo "EVM0 state=$state"
if test $state != "FlushFailed"
then
    echo "Test failed"
    exit 1
fi
...
echo "Stopping EVM..."
sendCmdToEVM Stop Draining
echo "Waiting 20 seconds for EVM to go to FlushFailed state"
sleep 20
state=`getParam EVM0_SOAP_HOST_NAME EVM0_SOAP_PORT rubuilder::evm::Application 0 stateName xsd:string`
echo "EVM0 state=$state"
if test $state != "FlushFailed"
